...
exports.check = function(input, importModule, opts) {
        ...
        body.p.Exprs.p.Items.forEach(function(funcExpr) {
            ...
            if (funcExpr.p.Value && funcIDS.indexOf(funcExpr.p.Value.p.Id) > - 1) {
                ...
                if (funcCallArg.p.Block) {
                    checkObject(funcCallArg, funcCall);
                    ...
                    body.p.Defs.p.Items.forEach(function(decl) {
                        if (decl.p.Id == varId) {
                            checkObject(decl.p.Value, funcCall);
                        }
                    });
                }
            }
    ...
    function checkObject(decl, funcDecl) {
        ...
        propsDecl.p.Exprs.p.Items.forEach(function(prop) {
            ...
            if (propName.toLowerCase().indexOf('content') > -1) {
                if (funcDecl.p.Name === 'nb-input' && propName === 'content') {
                    // ignores nb-input "content" prop becase it render as
                    // <input value="{.content}"/>
                    return;
                }
                    ...
                    if (someVal.p.Name) {
                        ...
                        if (propType === 'scalar' && !someVal.f.IsExternal && !someVal.f.IsUser) {
                            ...
                            while (c-- && parent && !found) {
                                ...
                                parent = parent.scope && parent.scope.parent;
                            }
    ...
    function checkScalarValue(prop, errors, propName, propType) {
        if (!prop.p.Value.p.Value) {
            if (prop.p.Value.__type !== 'xml') {
                errors.add(
                    Errors.TYPE.INVALID_VAR_TYPE,
                    {
                        propName: propName,
        ...
        var propValueNode = prop.p.Value.p.Value.p.Value.p;
        if ('Value' in propValueNode) {
            var value = propValueNode.Value;
            if (/<|>/i.test(value)) {
                errors.add(
        ...
        } else if ('Items' in propValueNode) {
            // throw only one error
            var found = false;
